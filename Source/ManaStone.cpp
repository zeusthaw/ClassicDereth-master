		...
		else // manastone being emptied into an item
		{
			...
			else
			{
				...
				if (manaStoneCurrentMana >= manaNeeded)
				{
					...
					SendText(csprintf("The %s gives %s mana to the %s.", GetName().c_str(), FormatNumberString(manaNeeded).c_str(), pTarget->GetName().c_str()), LTT_DEFAULT); //todo: made up message, confirm if it's correct
				...
				else
				{
					...
					SendText(csprintf("The %s gives %s mana to the %s.", GetName().c_str(), FormatNumberString(manaStoneCurrentMana).c_str(), pTarget->GetName().c_str()), LTT_DEFAULT); //todo: made up message, confirm if it's correct
